...
def load_view():
    ...
    st.markdown(f"<h5 style='text-align:center'>MLB At Bat Predictor - Recommendation</h5>", unsafe_allow_html=True)
    ...
    if st.button("Predict"):
        ...
        try:
            ...
            if pred == 1:
                text = (f"Winner is...the batter, <b>{hitter}</b>, with <b>{round(proba,2)}</b>% probability")
                st.write(f"<div style='text-align:center'><span style='font-size:26px'>{text}</span></div>", unsafe_allow_html=True)

            elif pred == 0:
                text = (f"Winner is...the pitcher, <b>{pitcher}</b>! <b>{hitter}</b> has a <b>{round(proba,2)}</b>% probability of success")
                st.write(f"<div style='text-align:center'><span style='font-size:26px'>{text}</span></div>", unsafe_allow_html=True)
            else:
